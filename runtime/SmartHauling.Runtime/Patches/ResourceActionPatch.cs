...
internal static class ResourceActionPatch
{
    ...
    private static void ReleasePendingPickupTargets(Goal goal, IGoapAgentOwner owner, ResourcePileInstance? currentPile)
    {
        if (currentPile != null)
        {
            MonoSingleton<ReservationManager>.Instance.ReleaseObject(currentPile, owner);
            MonoSingleton<ReservationManager>.Instance.ReleaseAll(currentPile);
        }
        ...
        foreach (var target in goal.GetTargetQueue(TargetIndex.A))
        {
            ...
            {
                MonoSingleton<ReservationManager>.Instance.ReleaseAll(queuedPile);
            }
        }
    }
